...
func newChapter(db *gorm.DB, opts ...gen.DOOption) chapter {
	...
	_chapter.ID = field.NewString(tableName, "id")
	_chapter.CourseID = field.NewString(tableName, "course_id")
	_chapter.Title = field.NewString(tableName, "title")
	_chapter.OrderIndex = field.NewInt32(tableName, "order_index")
...
type IChapterDo interface {
	...
	Unscoped() IChapterDo
	Create(values ...*model.Chapter) error
	CreateInBatches(values []*model.Chapter, batchSize int) error
	Save(values ...*model.Chapter) error
	First() (*model.Chapter, error)
	Take() (*model.Chapter, error)
	Last() (*model.Chapter, error)
	Find() ([]*model.Chapter, error)
	FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*model.Chapter, err error)
	FindInBatches(result *[]*model.Chapter, batchSize int, fc func(tx gen.Dao, batch int) error) error
	Pluck(column field.Expr, dest interface{}) error
	Delete(...*model.Chapter) (info gen.ResultInfo, err error)
	...
	FirstOrInit() (*model.Chapter, error)
	FirstOrCreate() (*model.Chapter, error)
	FindByPage(offset int, limit int) (result []*model.Chapter, count int64, err error)
